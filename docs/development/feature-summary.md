...
现在运行 `uv run timeflow add` 时，会默认进入**交互编辑模式**，让你可以：
...
```bash
uv run timeflow add --text "内容"
uv run timeflow add --image "图片.png"
```
...
**Q: 如何使用旧的行为（直接从剪贴板读取）？**  
A: 使用 `uv run timeflow add --no-edit`
...
```bash
alias quicklog='uv run timeflow add --no-edit'
```
